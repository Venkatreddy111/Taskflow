...
const SharePage = () => {
    ...
    useEffect(() => {
        const handleTaskData = (decodedTask) => {
            const task = { ...JSON.parse(decodedTask), id: generateUUID() };
    ...
    return (<>
      <Home />
      <Dialog open fullWidth slotProps={{
            paper: {
                style: {
                    borderRadius: "24px",
                    padding: " 10px 6px",
        ...
        {!error && taskData ? (<>
            <CustomDialogTitle title="Recieved Task" subTitle="You can now include it in your list." icon={<AddTaskRounded />}/>
            <DialogContent>
              <p style={{ fontSize: "16px", marginLeft: "6px" }}>
                <b translate={userName === "User" ? "yes" : "no"}>{userName}</b> shared you a task.
              </p>
              <TaskItem task={taskData} features={{
                enableLinks: false,
                enableGlow: false,
            }}/>
              {taskData && taskData.description && taskData.description.match(URL_REGEX) && (<Alert sx={{ mt: "20px" }} severity="warning">
            ...
            <CustomDialogTitle title="Failed to recieve Task" subTitle="This Task could not be processed." onClose={() => n("/")} icon={<ErrorRounded />}/>
            <DialogContent>
