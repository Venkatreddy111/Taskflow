...
import { DialogBtn, ManagementButton, ManagementButtonsContainer, ManagementContainer, ManagementHeader, TaskManagementContainer, } from "../styles";
import { UserContext } from "../contexts/UserContext";
import { Checkbox, Dialog, DialogActions, DialogContent, Divider, Tooltip, Typography, } from "@mui/material";
...
const Purge = () => {
    ...
    const { tasks } = user;
    const [selectedTasks, setSelectedTasks] = useStorageState([], "tasksToPurge", "sessionStorage"); // Array of selected task IDs
    const [deleteAllDialog, setDeleteAllDialog] = useState(false);
    useEffect(() => {
        document.title = "Taskflow - Purge tasks";
    }, []);
    ...
    const handlePurgeSelected = () => {
        const tasksToPurge = tasks.filter((task) => selectedTasks.includes(task.id));
    ...
    const renderTasks = (tasks, title) => {
        ...
        {tasks.map((task) => (<TaskManagementContainer key={task.id} backgroundClr={task.color} onClick={() => handleTaskClick(task.id)} selected={selectedTasks.includes(task.id)} translate="no">
    ...
    return (<>
      ...
      <ManagementContainer>
        ...
        {tasks.length === 0 && (<h3 style={{ opacity: 0.8, fontStyle: "italic" }}>You don't have any tasks to purge</h3>)}
      </ManagementContainer>
      <ManagementButtonsContainer>
        <Tooltip title={selectedTasks.length > 0 ? (<div>
      ...
      <Dialog open={deleteAllDialog} onClose={() => setDeleteAllDialog(false)}>
        <CustomDialogTitle title="Purge All Tasks" subTitle="Confirm that you want to purge all tasks" onClose={() => setDeleteAllDialog(false)} icon={<DeleteForeverRounded />}/>
        <DialogContent>
          This action cannot be undone. Are you sure you want to proceed?
        </DialogContent>
        <DialogActions>
          <DialogBtn onClick={() => setDeleteAllDialog(false)}>Cancel</DialogBtn>
          <DialogBtn color="error" onClick={() => {
            purgeTasks(tasks);
