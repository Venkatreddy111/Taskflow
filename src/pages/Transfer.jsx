...
import { FileDownload, FileUpload, IntegrationInstructionsRounded, Link as LinkIcon, PhonelinkRounded, QrCodeScannerRounded, } from "@mui/icons-material";
...
import { DropZone, ListContent, ManagementButton, ManagementButtonsContainer, ManagementContainer, ManagementHeader, TaskManagementContainer, VisuallyHiddenInput, } from "../styles";
import { Link, useNavigate } from "react-router-dom";
import QRCodeScannerDialog from "../components/QRCodeScannerDialog";
const Transfer = () => {
    const { user, setUser } = useContext(UserContext);
    const [selectedTasks, setSelectedTasks] = useStorageState([], "tasksToExport", "sessionStorage"); // Array of selected task IDs
    ...
    const handleExport = () => {
        ...
        Exported tasks:{" "}
        <ul>
          {tasksToExport.map((task) => (<li key={task.id}>
              <ListContent>
      ...
      </div>, { dismissButton: true, type: "blank" });
    };
    ...
    const handleImport = useCallback((taskFile) => {
        const file = taskFile;
        if (file) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const invalidTasks = importedTasks.filter((task) => {
                        const isInvalid = (task.name && task.name.length > TASK_NAME_MAX_LENGTH) ||
                    ...
                    if (invalidTasks.length > 0) {
                        ...
                        showToast(`These tasks cannot be imported due to exceeding maximum character lengths: ${invalidTaskNames}`, { type: "error" });
                        return;
                    }
                    const isCategoryColorValid = (category) => category.color && isHexColor(category.color);
                    const hasInvalidColors = importedTasks.some((task) => {
                        return ((task.color && !isHexColor(task.color)) ||
                    ...
                    if (file.size > maxFileSize) {
                        ...
                        showToast(`File size is too large (${formatMB.format(fileSizeMB)}/${formatMB.format(maxSizeMB)})`, { type: "error" });
                        return;
                    }
                ...
                <ul>
                  {importedTasks.map((task) => (<li key={task.id}>
                      <ListContent>
                        <Emoji unified={task.emoji || ""} size={20} emojiStyle={user.emojisStyle}/>
                        <span translate="no">{task.name}</span>
                      </ListContent>
                    ...
                    showToast(<div style={{ wordBreak: "break-all" }}>
    ...
    const handleImportFromQRCode = (qrCodeData) => {
        try {
    ...
    return (<>
      ...
      <ManagementContainer>
        {user.tasks.length > 0 ? (user.tasks.map((task) => (<TaskManagementContainer key={task.id} backgroundClr={task.color} onClick={() => handleTaskClick(task.id)} selected={selectedTasks.includes(task.id)} translate="no">
              <Checkbox color="primary" size="medium" checked={selectedTasks.includes(task.id)}/>
              <Typography variant="body1" component="span" sx={{ display: "flex", alignItems: "center", gap: "6px", wordBreak: "break-word" }}>
                <Emoji size={24} unified={task.emoji || ""} emojiStyle={user.emojisStyle}/>{" "}
                {task.name}
              </Typography>
            </TaskManagementContainer>))) : (<h3 style={{ opacity: 0.8, fontStyle: "italic" }}>You don't have any tasks to export</h3>)}
      </ManagementContainer>

      <ManagementButtonsContainer>
        <Tooltip title={selectedTasks.length > 0
            ? `Selected tasks: ${new Intl.ListFormat("en", {
                style: "long",
                type: "conjunction",
            }).format(selectedTasks.map((taskId) => {
        ...
        {systemInfo.os !== "Android" && systemInfo.os !== "iOS" && (<div style={{ width: "300px" }}>
            <DropZone onDragOver={handleDragOver} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop} isDragging={isDragging}>
              <FileUpload fontSize="large" color="primary"/>
              <div>Drop JSON file here to import tasks </div>
            </DropZone>
          </div>)}

        <Button component="label" role={undefined} tabIndex={-1} variant="outlined" sx={{
            p: "12px 20px",
            borderRadius: "14px",
            width: "300px",
        }}>
          <FileUpload /> &nbsp; Select JSON File
          <VisuallyHiddenInput accept=".json" type="file" ref={fileInputRef} onChange={handleSelectChange}/>
        </Button>
      ...
      <QRCodeScannerDialog subTitle="Import task by scanning a QR code" open={isScannerOpen} onClose={() => setIsScannerOpen(false)} onScan={(result) => {
