...
const TaskDetails = () => {
    ...
    useEffect(() => {
        document.title = `Taskflow - ${task?.name || "Task Details"}`;
    }, [task?.name]);
    if (!task) {
        return (<NotFound message={<div>
      ...
      <Container>
        ...
        <TaskTable>
          <tbody>
            ...
            {task?.sharedBy && (<TableRow>
                <TableHeader>Shared by: </TableHeader>
                <TableData>{task.sharedBy}</TableData>
              </TableRow>)}
            {task.category && task.category.length > 0 && (<TableRow>
                <TableHeader>Categories:</TableHeader>
                <TableData>
                  <CategoryContainer>
                    {task?.category?.map((category) => (<CategoryBadge key={category.id} category={category} glow={false}/>))}
                  </CategoryContainer>
                </TableData>
              </TableRow>)}
          </tbody>
