...
import { compressSyncData, decompressSyncData, extractOtherData, mergeSyncData, prepareSyncData, } from "../utils/syncUtils";
...
function mergeUserData(prevUser, mergedData, syncOption) {
    const updatedUser = {
        ...prevUser,
    ...
    if (syncOption === "other_device" && otherData) {
        ...
        const profilePictureData = otherData.profilePictureData;
...
export function usePeerSync() {
    ...
    const [syncStatus, setSyncStatus] = useState({ message: "", severity: "info" });
    const [otherDataSyncOption, setOtherDataSyncOption] = useState("this_device");
    const otherDataSyncOptionRef = useRef(otherDataSyncOption);
    const [otherDataSource, setOtherDataSource] = useState(null);
    useEffect(() => {
    ...
    const handleIncomingData = useCallback(async (rawData, syncOption, sendBack) => {
        try {
            ...
            const mergedData = mergeSyncData(user.tasks, user.deletedTasks, user.categories, user.deletedCategories, user.favoriteCategories, compressedDataForMerge, syncOption, localOtherData);
            ...
            if (sendBack) {
                const mergedSyncData = {
                    ...prepareSyncData(mergedData.tasks, mergedData.deletedTasks, mergedData.categories, mergedData.deletedCategories, mergedData.favoriteCategories, syncOption === "this_device" ? mergedData.otherData : undefined),
    ...
    const startHost = useCallback(() => {
        ...
        p.on("connection", (connection) => {
            ...
            connection.on("data", (rawData) => {
                handleIncomingData(rawData, otherDataSyncOptionRef.current, (data) => {
    ...
    const connectToHost = useCallback((hostId) => {
        ...
        p.on("open", () => {
            ...
            connection.on("open", async () => {
                ...
                const syncData = prepareSyncData(user.tasks, user.deletedTasks, user.categories, user.deletedCategories, user.favoriteCategories, otherData);
                const compressedData = compressSyncData(syncData);
                connection.send(compressedData);
                connection.on("data", (rawData) => {
                    handleIncomingData(rawData, "other_device");
                });
                connection.on("close", () => {
                    setStatus(STATUS.connectionClosed.message, STATUS.connectionClosed.severity);
                    resetAll();
