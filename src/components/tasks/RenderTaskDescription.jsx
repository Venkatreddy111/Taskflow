...
export const RenderTaskDescription = memo(({ task, textHighlighter = (text) => text, enableLinks = true, enableMoreButton = false, }) => {
    ...
    const descriptionWithLinks = truncatedParts.map((part, index) => {
        const isURL = index % 2 === 1;
        return isURL ? (<DescriptionLink key={index} url={part} color={color} disabled={!enableLinks} textHighlighter={textHighlighter}/>) : (<React.Fragment key={index}>{textHighlighter(part)}</React.Fragment>);
    });
    return (<>
        <ScreenDescription title={!isExpanded && shouldShowButton ? task.description : undefined}>
          {descriptionWithLinks}
          {!isExpanded && showMore && !task.done && "..."}
          {shouldShowButton && !task.done && (<ShowMoreBtn onClick={() => toggleShowMore(task.id)} clr={color}>
...
const DescriptionLink = memo(({ url, color, disabled = false, textHighlighter = (text) => text }) => {
    let domain = "";
    try {
    ...
    const LinkContent = (<StyledDescriptionLink id="task-description-link" clr={color} data-disabled={disabled} disabled={disabled}>
        {/* put a tag inside button to enable link preview on ios */}
        <a href={disabled ? undefined : url} rel={disabled ? undefined : "noreferrer"} target="_blank">
          <div>
            {disabled ? (<LinkOffRounded />) : (<FaviconImage draggable={false} alt="favicon" src={`https://www.google.com/s2/favicons?sz=96&domain_url=${url}`} style={{ width: 20, height: 20, marginRight: 2, borderRadius: 4 }}/>)}
...
`;
const StyledDescriptionLink = styled(Button) `
  margin: 0;
  color: ${({ clr }) => getFontColor(clr)};
  padding: 0 4px;
