...
import { CancelRounded, Close, Delete, DeleteRounded, DoneAll, Search, RadioButtonChecked, MoreVert, MoveUpRounded, } from "@mui/icons-material";
import { Box, Button, Dialog, DialogActions, DialogContent, IconButton, InputAdornment, Tooltip, } from "@mui/material";
...
import { NoTasks, RingAlarm, SearchClear, SearchInput, TaskActionContainer, TasksContainer, CategoriesListContainer, TaskNotFound, } from "./tasks.styled";
import { TaskMenu } from "./TaskMenu";
import { TaskIcon } from "../TaskIcon";
import { useToasterStore } from "react-hot-toast";
import { TaskSort } from "./TaskSort";
import { DndContext, closestCenter, DragOverlay, MeasuringStrategy, useSensors, useSensor, TouchSensor, MouseSensor, } from "@dnd-kit/core";
import { SortableContext, arrayMove, verticalListSortingStrategy } from "@dnd-kit/sortable";
import { restrictToVerticalAxis, restrictToParentElement } from "@dnd-kit/modifiers";
import DisabledThemeProvider from "../../contexts/DisabledThemeProvider";
const TaskMenuButton = memo(({ task, onClick }) => (<IconButton id="task-menu-button" aria-label="Task Menu" aria-controls="task-menu" aria-haspopup="true" aria-expanded={Boolean(task)} onClick={onClick} sx={{ color: getFontColor(task.color) }}>
      <MoreVert />
    </IconButton>));
/**
...
export const TasksList = () => {
    ...
    const { selectedTaskId, setSelectedTaskId, anchorEl, setAnchorEl, setAnchorPosition, search, setSearch, highlightMatchingText, multipleSelectedTasks, setMultipleSelectedTasks, handleSelectTask, editModalOpen, setEditModalOpen, deleteDialogOpen, setDeleteDialogOpen, sortOption, moveMode, setMoveMode, } = useContext(TaskContext);
    const open = Boolean(anchorEl);
    const [taskToDelete, setTaskToDelete] = useState(null);
    const [deleteSelectedOpen, setDeleteSelectedOpen] = useState(false);
    const [categories, setCategories] = useState(undefined);
    const [selectedCatId, setSelectedCatId] = useStorageState(undefined, "selectedCategory", "sessionStorage");
    ...
    const listFormat = useMemo(() => new Intl.ListFormat("en-US", {
        style: "long",
        type: "conjunction",
    }), []);
    ...
    const reorderTasks = useCallback((tasks) => {
        ...
        if (selectedCatId !== undefined) {
            const categoryFilter = (task) => task.category?.some((category) => category.id === selectedCatId) ?? false;
        ...
        const searchFilter = (task) => task.name.toLowerCase().includes(searchLower) ||
    ...
    const checkOverdueTasks = useCallback((tasks) => {
        if (location.pathname === "/share") {
            return;
        }
        const overdueTasks = tasks.filter((task) => task.deadline && new Date() > new Date(task.deadline) && !task.done);
        if (overdueTasks.length > 0) {
            const taskNames = overdueTasks.map((task) => task.name);
            showToast(<div translate="no" style={{ wordBreak: "break-word" }}>
    ...
    const dndKitSensors = useSensors(useSensor(MouseSensor), useSensor(TouchSensor, {
    ...
    return (<>
      ...
      <TasksContainer style={{ marginTop: user.settings.showProgressBar ? "0" : "24px" }}>
        {user.tasks.length > 0 && (<Box sx={{ display: "flex", alignItems: "center", gap: "10px", mb: "8px" }}>
            <DisabledThemeProvider>
              <SearchInput inputRef={searchRef} color="primary" placeholder="Search for task..." autoComplete="off" value={search} disabled={moveMode} onChange={(e) => {
                ...
                input: {
                    ...
                    endAdornment: search ? (<InputAdornment position="end">
                        <SearchClear color={orderedTasks.length === 0 && user.tasks.length > 0 ? "error" : "default"} onClick={() => setSearch("")}>
                          <Close sx={{
                            color: orderedTasks.length === 0 && user.tasks.length > 0
        ...
        {categories !== undefined && categories?.length > 0 && user.settings.enableCategories && (<CategoriesListContainer>
            {categories?.map((cat) => (<CategoryBadge key={cat.id} category={cat} emojiSizes={[24, 20]} list={"true"} label={<div>
                  ...
                  </div>} onClick={() => selectedCatId !== cat.id ? setSelectedCatId(cat.id) : setSelectedCatId(undefined)} onDelete={selectedCatId === cat.id ? () => setSelectedCatId(undefined) : undefined} deleteIcon={<CancelRounded />} sx={{
                    boxShadow: "none",
                    display: selectedCatId === undefined || selectedCatId === cat.id
                        ? "inline-flex"
                        : "none",
                    p: "20px 14px",
                    fontSize: "16px",
        ...
        {multipleSelectedTasks.length > 0 && (<TaskActionContainer>
            <div>
              <h3>
              ...
              <span translate="no" style={{ fontSize: "14px", opacity: 0.8 }}>
                {listFormat.format(multipleSelectedTasks
        ...
        {search && orderedTasks.length > 1 && user.tasks.length > 0 && (<div style={{
                textAlign: "center",
                fontSize: "18px",
                opacity: 0.9,
                marginTop: "12px",
        ...
        {/* FIXME: dry */}
        {user.tasks.length !== 0 ? (moveMode ? (<DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd} onDragStart={handleDragStart} modifiers={[restrictToVerticalAxis, restrictToParentElement]} measuring={{ droppable: { strategy: MeasuringStrategy.Always } }} sensors={dndKitSensors}>
              <SortableContext items={orderedTasks.map((task) => task.id)} strategy={verticalListSortingStrategy}>
                {orderedTasks.map((task) => (<TaskItem key={task.id} task={task} features={{
                    enableLinks: true,
                    ...
                    onDeselect: (taskId) => setMultipleSelectedTasks((prevTasks) => prevTasks.filter((id) => id !== taskId)),
                }} onContextMenu={(e) => {
                    handleClick(e, task.id);
                }} actions={<TaskMenuButton task={task} onClick={(event) => handleClick(event, task.id)}/>} blur={selectedTaskId !== task.id && open && !isMobile}/>))}
              </SortableContext>
              <DragOverlay dropAnimation={{
                ...
                {/* DRAG PREVIEW */}
                {activeDragId ? (<TaskItem task={orderedTasks.find((t) => t.id === activeDragId)} features={{
                    enableLinks: true,
                    enableGlow: user.settings.enableGlow,
                    enableSelection: false,
                    enableMoveMode: true,
                }} blur={false} actions={<TaskMenuButton task={orderedTasks.find((t) => t.id === activeDragId)} onClick={(event) => handleClick(event, orderedTasks.find((t) => t.id === activeDragId).id)}/>}/>) : null}
              </DragOverlay>
            </DndContext>) : (orderedTasks.map((task) => (<TaskItem key={task.id} task={task} features={{
                enableLinks: true,
                enableGlow: user.settings.enableGlow,
                enableSelection: true,
                enableMoveMode: true,
                ...
                handleClick(e, task.id);
            }} actions={<TaskMenuButton task={task} onClick={(event) => handleClick(event, task.id)}/>} blur={selectedTaskId !== task.id && open && !isMobile} textHighlighter={highlightMatchingText}/>)))) : (<NoTasks>
            <span>You don't have any tasks yet</span>
            <br />
            Click on the <span>+</span> button to add one
          </NoTasks>)}
        {search && orderedTasks.length === 0 && user.tasks.length > 0 ? (<TaskNotFound>
            <b>No tasks found</b>
            <br />
        ...
        <EditTask open={editModalOpen} task={user.tasks.find((task) => task.id === selectedTaskId)} onClose={() => setEditModalOpen(false)}/>
      </TasksContainer>
      <Dialog open={deleteDialogOpen} onClose={cancelDeleteTask}>
        <CustomDialogTitle title="Delete Task" subTitle="Are you sure you want to delete this task?" onClose={cancelDeleteTask} icon={<Delete />}/>
        <DialogContent>
          {taskToDelete && (<TaskItem task={taskToDelete} features={{
                enableGlow: false,
            }}/>)}
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={deleteSelectedOpen}>
        <CustomDialogTitle title="Delete selected tasks" subTitle="Confirm to delete selected tasks" icon={<DeleteRounded />}/>
