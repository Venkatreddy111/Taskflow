...
import { Cancel, Close, ContentCopy, DeleteRounded, Done, EditRounded, LaunchRounded, LinkRounded, MoveUpRounded, Pause, PlayArrow, PushPinRounded, RadioButtonChecked, RecordVoiceOver, RecordVoiceOverRounded, } from "@mui/icons-material";
...
export const TaskMenu = () => {
    ...
    const { selectedTaskId, anchorEl, anchorPosition, multipleSelectedTasks, handleSelectTask, setEditModalOpen, handleDeleteTask, handleCloseMoreMenu, moveMode, setMoveMode, setSearch, } = useContext(TaskContext);
    ...
    const handleMarkAsDone = () => {
        ...
        if (selectedTaskId) {
            ...
            <span>You've checked off all your TaskFlow items. Well done!</span>
                    ...
                    icon: (<div style={{ margin: "-6px 4px -6px -6px" }}>
    ...
    const handleReadAloud = () => {
        ...
        const taskDeadline = selectedTask?.deadline
            ? `. Task Deadline: ${calculateDateDifference(new Date(selectedTask.deadline), voice ? voice.lang : navigator.language)}`
            : "";
    ...
    const menuItems = [
      ...
      &nbsp; {selectedTask.pinned ? "Unpin" : "Pin"}
        ...
        ...(multipleSelectedTasks.length === 0
            ? [
                <StyledMenuItem key="select" onClick={() => handleSelectTask(selectedTaskId || generateUUID())} disabled={moveMode}>
        ...
        ...(!moveMode
            ? [
                <StyledMenuItem key="move" disabled={multipleSelectedTasks.length > 0} onClick={() => {
                        setMoveMode(true);
                        setSearch("");
                        handleCloseMoreMenu();
      ...
      <LaunchRounded /> &nbsp; Task details
        ...
        ...(settings.enableReadAloud && "speechSynthesis" in window
            ? [
                <StyledMenuItem key="read-aloud" onClick={handleReadAloud} disabled={window.speechSynthesis &&
                        (window.speechSynthesis.speaking || window.speechSynthesis.pending)}>
            <RecordVoiceOverRounded /> &nbsp; Read Aloud
          </StyledMenuItem>,
            ]
            : []),
        <StyledMenuItem key="share" onClick={() => {
      ...
      <LinkRounded /> &nbsp; Share
        ...
        <StyledMenuItem key="edit" onClick={() => {
      ...
      <ContentCopy /> &nbsp; Duplicate
        ...
        <StyledMenuItem key="delete" clr={ColorPalette.red} onClick={() => {
                handleDeleteTask();
                handleCloseMoreMenu();
            }}>
    ...
    const sheet = (<BottomSheet open={prefersReducedMotion ? true : Boolean(anchorEl)} onDismiss={handleCloseMoreMenu} snapPoints={({ minHeight, maxHeight }) => [minHeight, maxHeight]} expandOnContentDrag header={<div style={{
                textAlign: "left",
    ...
    return (<>
      ...
      {!isMobile && (<Menu id="task-menu" anchorEl={anchorEl} anchorPosition={anchorPosition ? anchorPosition : undefined} anchorReference={anchorPosition ? "anchorPosition" : undefined} open={Boolean(anchorEl)} onClose={handleCloseMoreMenu} sx={{
                "& .MuiPaper-root": {
                    borderRadius: "18px",
                    minWidth: "200px",
                    boxShadow: "none",
      ...
      <ShareDialog open={showShareDialog} onClose={() => setShowShareDialog(false)} selectedTask={selectedTask}/>
    </>);
};
