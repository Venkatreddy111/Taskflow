...
import { AccessTimeRounded, CalendarTodayRounded, MoveUpRounded, SortByAlphaRounded, } from "@mui/icons-material";
...
export const TaskSort = () => {
    const { sortOption, setSortOption, sortAnchorEl, setSortAnchorEl, moveMode } = useContext(TaskContext);
    ...
    return (<>
      <SortButton onClick={handleSortClick} aria-controls={sortOpen ? "sort-menu" : undefined} aria-haspopup="true" aria-expanded={sortOpen ? "true" : undefined} isMenuOpen={sortOpen} disabled={moveMode}>
      ...
      <Menu id="sort-menu" anchorEl={sortAnchorEl} open={sortOpen} onClose={() => handleSortClose()} slotProps={{
            list: {
                "aria-labelledby": "sort-button",
            },
        }} transformOrigin={{ horizontal: "right", vertical: "top" }} anchorOrigin={{ horizontal: "right", vertical: "bottom" }} sx={{
            "& .MuiPaper-root": {
                borderRadius: "18px",
                minWidth: "200px",
                boxShadow: "none",
        ...
        {sortOptions.map(({ value, label, icon }) => (<StyledMenuItem key={value} onClick={() => handleSortClose(value)} selected={sortOption === value}>
