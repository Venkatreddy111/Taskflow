...
import { Alert, AlertTitle, Box, Button, Dialog, DialogActions, DialogContent, InputAdornment, Tab, Tabs, TextField, } from "@mui/material";
import { CustomDialogTitle, TaskItem } from "..";
import { Apple, CalendarTodayRounded, ContentCopyRounded, DownloadRounded, IosShare, LinkRounded, QrCode2Rounded, } from "@mui/icons-material";
...
export const ShareDialog = ({ open, onClose, selectedTask }) => {
    const { user } = useContext(UserContext);
    const { settings, name } = user;
    const { toasts } = useToasterStore();
    const [shareTabVal, setShareTabVal] = useState(0);
    const tabs = [
    ...
    const generateShareableLink = (task, userName) => {
        if (task) {
            const taskToShare = {
                ...task,
                sharedBy: undefined,
    ...
    const handleAddToAppleCalendar = () => {
        if (!selectedTask)
            return;
        const deadline = formatICSDate(selectedTask.deadline ? new Date(selectedTask.deadline) : new Date());
    ...
    return (<Dialog open={open} onClose={onClose} slotProps={{
      ...
      <CustomDialogTitle title="Share Task" subTitle="Share your task with others." onClose={onClose} icon={<IosShare />}/>
      <DialogContent>
        <TaskItem task={selectedTask} features={{
            enableGlow: false,
        }}/>
        <Tabs value={shareTabVal} onChange={(_event, newValue) => setShareTabVal(newValue)} sx={{ m: "8px 0" }}>
          {tabs.map((tab) => (<StyledTab key={tab.label} label={tab.label} icon={tab.icon} disabled={tab.disabled} totaltabs={tabs.length}/>))}
        </Tabs>
        <TabGroupProvider name="share" value={shareTabVal}>
          <TabPanel index={0}>
            <ShareField value={generateShareableLink(selectedTask, name || "User")} fullWidth variant="outlined" label="Shareable Link" slotProps={{
            input: {
                readOnly: true,
                ...
                endAdornment: (<InputAdornment position="end">
                      <Button onClick={handleCopyToClipboard} sx={{ p: "12px", borderRadius: "14px", mr: "4px" }}>
          ...
          <TabPanel index={1}>
            <QRCodeContainer>
              <QRCode id="QRCodeShare" value={generateShareableLink(selectedTask, name || "User")} size={350} style={{
            borderRadius: "8px",
            padding: "8px",
          ...
          {systemInfo.isAppleDevice && (<TabPanel index={2}>
              <Box sx={{
                mt: "22px",
                display: "flex",
        ...
        {shareTabVal !== 2 && (<Alert severity="info" sx={{ mt: "20px" }} 
...
const QRCodeContainer = styled.div `
  ...
  margin-top: 22px;
`;
const UnstyledTab = ({ ...props }) => <Tab iconPosition="start" {...props}/>;
