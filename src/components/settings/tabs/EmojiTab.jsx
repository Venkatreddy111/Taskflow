...
import { Alert, AlertTitle, Button, Card, CardContent, Divider, Typography, Stack, Box, } from "@mui/material";
...
export default function EmojiTab() {
    ...
    const [emojiStyleValue, setEmojiStyleValue] = useState(user.emojisStyle);
    const [hasEmojiData, setHasEmojiData] = useState(!!localStorage.getItem("epr_suggested"));
    ...
    return (<Stack spacing={3}>
      ...
      <Card elevation={2} sx={{ borderRadius: "16px" }}>
        <CardContent>
          ...
          <CustomRadioGroup options={emojiStyles} value={emojiStyleValue} onChange={(val) => {
            setEmojiStyleValue(val);
            setUser((prev) => ({ ...prev, emojisStyle: val }));
        }} disabledOptions={isOnline ? [] : offlineDisabled}/>

          {!isOnline && (<Alert severity="warning" sx={{ mt: 2, borderRadius: "12px" }} icon={<WifiOffRounded />}>
      ...
      <Card elevation={2} sx={{ borderRadius: "16px" }}>
        <CardContent>
          ...
          <CustomSwitch settingKey="simpleEmojiPicker" header="Enable Simple Picker" text="Show only your recently used emojis for quicker access." disabled={!hasEmojiData} disabledReason="No recent emoji data found."/>
        </CardContent>
      </Card>
      ...
      <Card elevation={2} sx={{ borderRadius: "16px" }}>
        <CardContent>
          ...
          <Box display="flex" justifyContent="flex-start">
            <Button variant="contained" color="error" startIcon={<DeleteRounded />} onClick={() => {
