...
import { AddReaction, AutoAwesome, Edit, EmojiEmotions, RemoveCircleOutline, } from "@mui/icons-material";
import { Alert, Avatar, Badge, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, Tooltip, } from "@mui/material";
...
export const CustomEmojiPicker = ({ emoji, setEmoji, color, name, type }) => {
    ...
    const getFrequentlyUsedEmojis = () => {
        const frequentlyUsedEmojis = JSON.parse(localStorage.getItem("epr_suggested") || "null");
        if (!frequentlyUsedEmojis) {
            return [];
        }
        frequentlyUsedEmojis.sort((a, b) => b.count - a.count);
        const topEmojis = frequentlyUsedEmojis.slice(0, 6);
        const topUnified = topEmojis.map((item) => item.unified);
        return topUnified;
    };
    ...
    async function useAI() {
        ...
        setIsAILoading(true);
        try {
            const sessionInstance = session || (await LanguageModel.create());
            // chrome://flags/#text-safety-classifier must be disabled to make this prompt work
            const response = await sessionInstance.prompt(`Respond with ONLY ONE emoji that best represents this task: "${name}". DO NOT include any other text, explanations, or symbols. Just the SINGLE emoji.`);
            console.log("Full AI response:", response);
            // this doesn't split emojis into separate characters
            const emojiRegex = /\p{RI}\p{RI}|\p{Emoji}(\p{EMod}+|\u{FE0F}\u{20E3}?|[\u{E0020}-\u{E007E}]+\u{E007F})?(\u{200D}\p{Emoji}(\p{EMod}+|\u{FE0F}\u{20E3}?|[\u{E0020}-\u{E007E}]+\u{E007F})?)+|\p{EPres}(\p{EMod}+|\u{FE0F}\u{20E3}?|[\u{E0020}-\u{E007E}]+\u{E007F})?|\p{Emoji}(\p{EMod}+|\u{FE0F}\u{20E3}?|[\u{E0020}-\u{E007E}]+\u{E007F})/gu;
          ...
          <br />
          {String(error)}
        </div>, { type: "error", duration: 8000 });
        }
        finally {
            setIsAILoading(false);
            const end = new Date().getTime();
            console.log(`%cTook ${end - start}ms to generate.`, `color: ${end - start > 1500 ? "orange" : "lime"}`);
        }
    }
    ...
    const renderAvatarContent = () => {
        ...
        if (currentEmoji) {
            const emojiSize = emojisStyle === EmojiStyle.NATIVE && systemInfo.os === "iOS"
                ? 64
                : emojisStyle === EmojiStyle.NATIVE
                    ? 48
                    : 64;
            return (<EmojiElement key={currentEmoji}>
          <Emoji size={emojiSize} emojiStyle={emojisStyle} unified={currentEmoji}/>
        </EmojiElement>);
        }
        else {
            return (<AddReaction sx={{
                    fontSize: "52px",
                    color: fontColor,
                    transition: ".3s all",
                }}/>);
    ...
    return (<>
      <EmojiContainer>
        <Badge overlap="circular" anchorOrigin={{ vertical: "bottom", horizontal: "right" }} badgeContent={<EditBadge onClick={toggleEmojiPicker} tabIndex={0} role="button" onKeyDown={(e) => {
      ...
      {"LanguageModel" in window && name !== undefined && (<Tooltip title={!name ? `Enter a name for the ${type} to find emoji` : undefined}>
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
            <DisabledThemeProvider>
              <Button onClick={useAI} disabled={name?.length < 3 ||
                (type === "task"
      ...
      {/* Simple Emoji Picker */}
      {showEmojiPicker && settings.simpleEmojiPicker && (<SimplePickerContainer>
          <Suspense fallback={<CircularProgress size={40} thickness={5}/>}>
            <EmojiPicker style={{ border: "none" }} reactionsDefaultOpen reactions={getFrequentlyUsedEmojis()} emojiStyle={emojisStyle} onReactionClick={handleEmojiClick} allowExpandReactions={false} theme={emotionTheme.darkmode ? Theme.DARK : Theme.LIGHT} autoFocusSearch={false}/>
          </Suspense>
          {currentEmoji && (<Button onClick={handleRemoveEmoji} fullWidth variant="outlined" color="error">
              <RemoveCircleOutline /> &nbsp; Remove Emoji
            </Button>)}
        </SimplePickerContainer>)}

      {showEmojiPicker && !settings.simpleEmojiPicker && (<>
          <Dialog open={showEmojiPicker} onClose={toggleEmojiPicker} slotProps={{
                paper: {
                    style: {
                        padding: "12px",
                        borderRadius: "24px",
            ...
            <CustomDialogTitle title="Choose Emoji" subTitle={`Choose the perfect emoji for your ${type}.`} onClose={toggleEmojiPicker} icon={<AddReaction />}/>
            <DialogContent sx={{ p: 0, m: 0 }}>
              {!isOnline && emojisStyle !== EmojiStyle.NATIVE && (<Box sx={{ mx: "14px", mb: "16px" }}>
                  <Alert severity="warning">
                    Emojis may not load correctly when offline. Try switching to the native emoji
                    style.
                  </Alert>
                  <Button variant="outlined" color="warning" fullWidth sx={{ mt: "14px" }} onClick={() => {
                    setUser((prevUser) => ({
                        ...prevUser,
                        emojisStyle: EmojiStyle.NATIVE,
                    }));
              ...
              <EmojiPickerContainer>
                <Suspense fallback={!settings.simpleEmojiPicker && (<PickerLoader pickerTheme={emotionTheme.darkmode ? "dark" : "light"}></PickerLoader>)}>
                  <EmojiPicker width="100vw" height="550px" lazyLoadEmojis emojiStyle={emojisStyle} theme={emotionTheme.darkmode ? Theme.DARK : Theme.LIGHT} suggestedEmojisMode={SuggestionMode.FREQUENT} autoFocusSearch={false} onEmojiClick={handleEmojiClick} searchPlaceHolder="Search emoji" previewConfig={{
                defaultEmoji: "1f4dd",
            ...
            <DialogActions>
              {currentEmoji && (<DialogBtn color="error" onClick={handleRemoveEmoji}>
                  <RemoveCircleOutline /> &nbsp; Remove Emoji
                </DialogBtn>)}
