...
import { AddRounded, EditRounded, ExpandMoreRounded, RadioButtonChecked, StarRounded, } from "@mui/icons-material";
import { Box, Button, Divider, FormControl, FormLabel, MenuItem, Select, useTheme, ListSubheader, } from "@mui/material";
 ...
 */
export const CategorySelect = ({ selectedCategories, onCategoryChange, width, fontColor, }) => {
    ...
    const favoriteCats = categories.filter((cat) => favoriteCategories && favoriteCategories.includes(cat.id));
    const otherCats = categories.filter((cat) => !favoriteCategories || !favoriteCategories.includes(cat.id));
    return (<FormControl sx={{ width: width || "100%" }}>
      <FormLabel sx={{
      ...
      <StyledSelect multiple width={width} value={selectedCats.map((cat) => cat.id)} onChange={handleCategoryChange} open={isOpen} onOpen={() => setIsOpen(true)} onClose={() => setIsOpen(false)} IconComponent={() => (<Box sx={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={() => setIsOpen((prev) => !prev)}>
            <ExpandMoreRounded sx={{
                marginRight: "14px",
                color: fontColor || ColorPalette.fontLight,
                transform: isOpen ? "rotate(180deg)" : "none",
            }}/>
          </Box>)} displayEmpty renderValue={() => selectedCats.length > 0 ? (<Box sx={{ display: "flex", flexWrap: "wrap", gap: "4px 8px" }}>
              {selectedCats.map((category) => (<CategoryBadge key={category.id} category={category} sx={{ cursor: "pointer" }} glow={false}/>))}
            </Box>) : (<Box sx={{ color: fontColor }}>Select Categories</Box>)} MenuProps={{
            PaperProps: {
                style: {
                    maxHeight: 450,
                    zIndex: 999999,
        ...
        {(() => {
            const renderCategoryItem = (category) => (<CategoriesMenu key={category.id} value={category.id} clr={category.color} tabIndex={0} translate="no" disable={selectedCats.length >= MAX_CATEGORIES_IN_TASK &&
            ...
            const createCategoryGroup = (cats, headerText, headerId) => {
                if (cats.length === 0)
                    return [];
                return [
            ...
            if (categories && categories.length > 0) {
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                  <b>
                    ...
                    <span style={{
                            transition: ".3s color",
                            color: selectedCats.length >= MAX_CATEGORIES_IN_TASK
                                ? "#f34141"
                                : "currentcolor",
                        }}>
                  ...
                  <SelectedNames>
                    Selected:{" "}
                    {selectedCats.length > 0 ? (new Intl.ListFormat("en", {
                            style: "long",
                            type: "conjunction",
                        }).format(selectedCats.map((category) => category.name))) : (<span style={{ fontStyle: "italic" }}>none</span>)}
                  </SelectedNames>
                </div>
              </HeaderMenuItem>,
                    ...createCategoryGroup(favoriteCats, <>
                  <StarRounded color="warning" sx={{ fontSize: "18px" }}/>
                  &nbsp;Favorite Categories
                </>, "header-favorites"),
                    ...createCategoryGroup(otherCats, favoriteCats.length > 0 ? "Other Categories" : "", "header-others"),
...
`;
const CategoriesMenu = styled(MenuItem) `
  padding: 12px 16px;
  border-radius: 16px;
  margin: 8px;
  display: flex;
