import { AddRounded, CategoryRounded, GetAppRounded, PersonRounded, TaskAlt, } from "@mui/icons-material";
import { Badge, BottomNavigation, BottomNavigationAction, Box, css, styled, useTheme, } from "@mui/material";
...
export const BottomNav = () => {
    ...
    return (<Container>
      <StyledBottomNavigation showLabels glow={settings.enableGlow} value={value} onChange={(_event, newValue) => {
            window.scrollTo({
                top: 0,
                behavior: "smooth",
            });
        ...
        <NavigationButton onClick={() => n("/")} label="Tasks" icon={<Badge color="primary" badgeContent={value !== 0 ? tasks.filter((task) => !task.done).length : undefined} max={99}>
              <TaskAlt sx={{ fontSize: smallIconSize }}/>
            </Badge>}/>
        <NavigationButton onClick={() => n("/categories")} label="Categories" icon={<CategoryRounded sx={{ fontSize: smallIconSize }}/>} disabled={!settings.enableCategories}/>

        <NavigationButton onClick={() => n("add")} showLabel={false} aria-label="Add" icon={<AddIconContainer clr={theme.palette.primary.main} animate={tasks.length === 0 && value !== 2}>
              <AddIcon clr={theme.palette.primary.main} fontSize="large"/>
            </AddIconContainer>}/>
        <NavigationButton onClick={() => n("transfer")} label="Transfer" icon={<GetAppRounded sx={{ fontSize: smallIconSize }}/>}/>
