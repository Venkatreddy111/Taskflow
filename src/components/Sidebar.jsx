...
import { AccessTimeFilledRounded, AddRounded, AdjustRounded, BugReportRounded, CategoryRounded, DeleteForeverRounded, DownloadDoneRounded, Favorite, FiberManualRecord, GetAppRounded, GitHub, InstallDesktopRounded, InstallMobileRounded, IosShareRounded, Logout, PhoneIphoneRounded, PhonelinkRounded, SettingsRounded, StarRounded, TaskAltRounded, ThumbUpRounded, } from "@mui/icons-material";
import { Dialog, DialogActions, DialogContent, Divider, IconButton, MenuItem, SwipeableDrawer, Tooltip, } from "@mui/material";
...
import { getProfilePictureFromDB, shortRelativeTime, showToast, systemInfo, timeAgo, } from "../utils";
export const ProfileSidebar = () => {
    ...
    const [openInstalledDialog, setOpenInstalledDialog] = useState(false);
    useEffect(() => {
    ...
    return (<Container>
      <Tooltip title={<div translate={name ? "no" : "yes"}>{name || "User"}</div>}>
        <IconButton aria-label="Sidebar" aria-controls={open ? "basic-menu" : undefined} aria-haspopup="true" aria-expanded={open ? "true" : undefined} onClick={handleClick} sx={{ zIndex: 1 }}>
          <UserAvatar src={avatarSrc || undefined} alt={name || "User"} hasimage={profilePicture !== null} pulse={user.name === defaultUser.name &&
            user.profilePicture === defaultUser.profilePicture &&
            JSON.stringify(user.settings) === JSON.stringify(defaultUser.settings)} size="52px" onError={() => {
      ...
      <StyledSwipeableDrawer disableBackdropTransition={systemInfo.os !== "iOS"} disableDiscovery={systemInfo.os === "iOS"} id="basic-menu" anchor="right" open={open} onOpen={(e) => e.preventDefault()} onClose={handleClose}>
        <LogoContainer translate="no" onClick={() => {
            n("/");
            handleClose();
        }}>
        ...
        <MenuLink to="/">
          <StyledMenuItem onClick={handleClose}>
            ...
            {tasks.filter((task) => !task.done).length > 0 && (<Tooltip title={`${tasks.filter((task) => !task.done).length} tasks to do`}>
                <MenuLabel>
        ...
        {settings.enableCategories !== undefined && settings.enableCategories && (<MenuLink to="/categories">
            <StyledMenuItem onClick={handleClose}>
        ...
        <MenuLink to="/sync">
          <StyledMenuItem onClick={handleClose}>
            ...
            {user.lastSyncedAt && (<Tooltip title={`Last synced ${timeAgo(new Date(user.lastSyncedAt))}`}>
                <MenuLabel>
                  <span>
        ...
        <MenuLink to="https://github.com/Venkatreddy111/Taskflow">
          <StyledMenuItem translate="no">
            ...
            {stars && (<Tooltip title={`${stars} stars on Github`}>
        ...
        <MenuLink to="https://github.com/Venkatreddy111/Taskflow/issues/new">
          <StyledMenuItem>
            ...
            {Boolean(issuesCount || issuesCount === 0) && (<Tooltip title={`${issuesCount} open issues`}>
        ...
        {supportsPWA && !isAppInstalled && (<StyledMenuItem tabIndex={0} onClick={installPWA}>
            {systemInfo.os === "Android" ? (<InstallMobileRounded />) : (<InstallDesktopRounded className="InstallDesktopRoundedIcon"/>)}
            &nbsp; Install App
          </StyledMenuItem>)}

        {systemInfo.browser === "Safari" &&
            systemInfo.os === "iOS" &&
            !window.matchMedia("(display-mode: standalone)").matches && (<StyledMenuItem tabIndex={0} onClick={() => {
                showToast(<div style={{ display: "inline-block" }}>
        ...
        <StyledMenuItem tabIndex={0} onClick={() => {
            handleClose();
        ...
        <ProfileOptionsBottom>
          <SettingsMenuItem tabIndex={0} onClick={() => {
          ...
          <MenuLink to="/user">
            <ProfileMenuItem translate={name ? "no" : "yes"} onClick={handleClose}>
              <UserAvatar src={avatarSrc || undefined} hasimage={profilePicture !== null} size="44px">
                {name ? name[0].toUpperCase() : undefined}
              </UserAvatar>
          ...
          <CreditsContainer translate="no">
            ...
            <a style={{ textDecoration: "none", color: "inherit" }} href="https://github.com/Venkatreddy111">
              Venkatreddy111
            </a>
          </CreditsContainer>
          <CreditsContainer>
            {lastUpdate && (<Tooltip title={timeAgo(new Date(lastUpdate))}>
                <span>
                  Last update:{" "}
      ...
      <Dialog open={openInstalledDialog} onClose={() => setOpenInstalledDialog(false)}>
        <CustomDialogTitle title="App installed successfully!" subTitle="The app is now running as a PWA." icon={<DownloadDoneRounded />} onClose={() => setOpenInstalledDialog(false)}/>
        <DialogContent>
      ...
      <SettingsDialog open={openSettings} onClose={() => setOpenSettings(false)} handleOpen={() => setOpenSettings(true)}/>
    </Container>);
};
const MenuLink = ({ to, children }) => {
    const styles = { borderRadius: "14px" };
    if (to.startsWith("/") || to === "") {
        return (
    ...
    return (<a href={to} target="_blank" style={styles}>
      {children}
    </a>);
};
const PulseMenuLabel = () => {
    return (<StyledPulseMenuLabel>
      <div style={{
            display: "flex",
            alignItems: "center",
