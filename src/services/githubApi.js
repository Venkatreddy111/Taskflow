        ...
        if (repoResponse.ok && branchResponse.ok) {
            ...
            if (repoResponse.status === 403 && branchResponse.status === 403) {
                if (process.env.NODE_ENV === "development")
                    return EMPTY_GITHUB_INFO; // Skip in dev mode
            ...
            throw new Error(`Failed to fetch repository or branch information: ${repoResponse.status}, ${branchResponse.status}`);
        }
    }
    catch (error) {
