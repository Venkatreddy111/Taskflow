...
export const getProfilePictureFromDB = async (profilePicture) => {
    if (!profilePicture)
        return null;
...
export const saveProfilePictureInDB = async (base64Image) => {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open(DB_NAME, 1);
        request.onsuccess = (event) => {
            const db = event.target.result;
...
export const deleteProfilePictureFromDB = async () => {
    return new Promise((resolve, reject) => {
 ...
 */
export function optimizeProfilePicture(file) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        ...
        img.onload = () => {
            ...
            if (!ctx)
                return reject("Canvas context failed");
...
export const validateImageFile = (file) => {
    ...
    if (!ALLOWED_PFP_TYPES.includes(file.type)) {
        const allowedFormats = ALLOWED_PFP_TYPES.map((type) => type.replace("image/", "").toUpperCase()).join(", ");
