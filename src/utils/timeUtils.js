...
const getDayOfWeek = (date) => date.toLocaleDateString(getLocale(), { weekday: "long" });
const formatTime = (date) => date.toLocaleTimeString(getLocale(), { hour: "2-digit", minute: "2-digit" });
const formatDateOnly = (date) => date.toLocaleDateString(getLocale(), {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
});
 ...
 */
export const timeAgo = (input, lang = getLocale()) => {
    const now = new Date();
 ...
 */
export const calculateDateDifference = (date, lang = navigator.language || "en-US") => {
    const now = new Date();
