...
export async function extractOtherData(user) {
 ...
 */
function mergeTasks(localTasks, remoteTasks, localDeleted, remoteDeleted, localDeletedCategories, remoteDeletedCategories) {
    ...
    const processedTasks = [...localTasks, ...remoteTasks]
        ...
        if (task.category) {
            const filteredCategories = task.category.filter((cat) => !allDeletedCategories.has(cat.id));
 ...
 */
function mergeCategories(localCategories, remoteCategories, localDeleted, remoteDeleted) {
 ...
 */
function mergeFavoriteCategories(localFavorites, remoteFavorites, localCategories, remoteCategories) {
 ...
 */
export function prepareSyncData(tasks, deletedTasks, categories, deletedCategories, favoriteCategories, otherData) {
    return {
        version: 1,
        tasks,
        deletedTasks,
 ...
 */
export function compressSyncData(data) {
...
export function decompressSyncData(compressed) {
    try {
        ...
        data.tasks.forEach((task) => {
            if (task.lastSave)
                task.lastSave = new Date(task.lastSave);
            if (task.date)
 ...
 */
export function mergeSyncData(localTasks, localDeletedTasks, localCategories, localDeletedCategories, localFavoriteCategories, syncDataStr, syncOption, localOtherData) {
    const syncData = JSON.parse(LZString.decompressFromEncodedURIComponent(syncDataStr));
    // merge tasks
    const mergedTasks = mergeTasks(localTasks, syncData.tasks, localDeletedTasks, syncData.deletedTasks, localDeletedCategories, syncData.deletedCategories);
    // merge categories
    const mergedCategories = mergeCategories(localCategories, syncData.categories, localDeletedCategories, syncData.deletedCategories);
    // Store the combined deleted items
    const mergedDeletedTasks = Array.from(new Set([...localDeletedTasks, ...syncData.deletedTasks]));
    const mergedDeletedCategories = Array.from(new Set([...localDeletedCategories, ...syncData.deletedCategories]));
    ...
    const finalFavoriteCategories = mergeFavoriteCategories(localFavoriteCategories, syncData.favoriteCategories || [], finalCategories, syncData.categories);
    // merge/select otherData
    let mergedOtherData = undefined;
